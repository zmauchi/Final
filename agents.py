...
class SegAgent(Agent):
    ...
    def step(self):
       ...
       # wealth gain and loss based on the mean wealth of neighbors
        ...
        if self.type==1:
            #average housing appreciation rate and discrimination effect in housing markets
            self.wealth += self.neighborwealth * 0.04 * ((self.similar+1) / (len(self.model.grid.get_neighbors(self.pos, True))+1))
        else:
            self.wealth += self.neighborwealth * 0.04 * (((len(self.model.grid.get_neighbors(self.pos, True)))-self.similar-1) / (len(self.model.grid.get_neighbors(self.pos, True))+1))
        self.wealth-=self.neighborwealth*0.02 #pay property tax applied to a neighborhood
